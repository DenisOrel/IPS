...
// Type: Intermech.XmlExchange.IpsXml.Converter.TechcardToIps.Config.Configuration.Load.Loaders.ParamValueConfigLoaders.ValueConfigsLoader
...
[ConfigLoader(NodeType.ValueConfigs)]
internal class ValueConfigsLoader(ConfigLoadService loadersService, IpsXmlLogger logger) : 
  BaseConfigLoader<ValueConfigs>(loadersService, logger)
{
  protected override void OnLoadAddParams(ValueConfigs target, XElement source)
  {
    this.Logger.Info(LocalizationHolder.rm.GetString("msg_load_relation_configs"));
    foreach (XElement element in source.Elements())
    {
      BaseConfig baseConfig = this.LoadersService.LoadConfig(element);
      if (baseConfig is ValueConfig && !string.IsNullOrEmpty(baseConfig.Id))
      {
        if (target[baseConfig.Id] != null)
          ...
          target[baseConfig.Id] = baseConfig as ValueConfig;
      }
    }
    if (target.Count == 0)
    ...
    this.Logger.Info(LocalizationHolder.rm.GetString("msg_load_relation_configs_complete"));
  }
}
