...
// Type: Intermech.XmlExchange.IpsXml.Converter.TechcardToIps.Config.Configuration.Load.Loaders.RelationConfigsLoader
...
[ConfigLoader(NodeType.RelationConfigs)]
internal class RelationConfigsLoader(ConfigLoadService loadersService, IpsXmlLogger logger) : 
  BaseConfigLoader<RelationConfigs>(loadersService, logger)
{
  protected override void OnLoadAddParams(RelationConfigs target, XElement source)
  {
    ...
    foreach (XElement element in source.Elements())
    {
      ...
      if (baseConfig is RelationConfig && !string.IsNullOrEmpty(baseConfig.Id))
      {
        if (target[baseConfig.Id] != null)
          ...
          target[baseConfig.Id] = baseConfig as RelationConfig;
      }
    }
    if (target.Count == 0)
