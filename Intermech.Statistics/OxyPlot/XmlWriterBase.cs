...
public abstract class XmlWriterBase : IDisposable
{
  ...
  {
    this.w = XmlWriter.Create(stream, new XmlWriterSettings()
    {
      Indent = true,
  ...
  public virtual void Close() => this.Flush();
  ...
  protected void WriteEndDocument() => this.w.WriteEndDocument();

  protected void WriteEndElement() => this.w.WriteEndElement();

  protected void WriteRaw(string text) => this.w.WriteRaw(text);

  protected void WriteStartDocument(bool standalone) => this.w.WriteStartDocument(standalone);

  protected void WriteStartElement(string name) => this.w.WriteStartElement(name);

  protected void WriteStartElement(string name, string ns) => this.w.WriteStartElement(name, ns);

  protected void WriteString(string text) => this.w.WriteString(text);
