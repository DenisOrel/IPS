...
// Type: OxyPlot.Reporting.WikiReportWriter
...
public class WikiReportWriter : StreamWriter, IReportWriter
{
  ...
  public void WriteHeader(Header h)
  {
    ...
    for (int index = 0; index < h.Level; ++index)
      empty += "!";
    this.WriteLine($"{empty} {h.Text}");
  }
  ...
  public void WriteTable(Table t)
  {
    ...
    foreach (TableRow row in (IEnumerable<TableRow>) t.Rows)
    {
      ...
      {
        string content = row.Cells[index].Content;
        bool isHeader = row.IsHeader || t.Columns[index].IsHeader;
        this.Write(WikiReportWriter.GetCellText(index, count, WikiReportWriter.PadString(content, t.Columns[index].Alignment, numArray[index]), isHeader));
      }
      this.WriteLine();
    }
    this.WriteLine();
