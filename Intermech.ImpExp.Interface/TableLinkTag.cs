...
// Type: Intermech.ImpExp.Interface.TableLinkTag
...
public class TableLinkTag : ITagImportObject
{
  ...
  public TableLinkTag(int catalogKey, int level)
  {
  ...
  public byte[] Save()
  {
    using (MemoryStream output = new MemoryStream())
    {
      ...
      try
      {
        binaryWriter.Write(this.CatalogKey);
  ...
  public void Load(byte[] s)
  {
    using (MemoryStream input = new MemoryStream(s))
    {
      ...
      try
      {
        this.CatalogKey = binaryReader.ReadInt32();
  ...
  public virtual short ClassID => 20;
}
