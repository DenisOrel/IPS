...
// Type: Intermech.Project.Server.DBDependencyCollection
...
public class DBDependencyCollection([NotNull] UserSession uSession, [NotEmpty] int objectType) : 
  DBObjectCollection(uSession, objectType),
  IDBDependencyCollection,
  IDBObjectCollection,
  IDBRecords,
  IDBSessionable,
  IDBAttributableCollection
{
  ...
  [NotEmpty]
  public static int TypeID => (int) (IpsMetadataEntityBase<int>) Intermech.Project.ObjectTypes.Dependency;

  [NotNull]
  internal DBDependency CreateDependencyCopy([NotNull] IDBObject prototype)
  {
    return this.Create(prototype).CastToClass<DBDependency>();
  }

  [NotNull]
  internal DBDependency CreateDependency(
    [NotEmpty] long id,
    ...
    Guid versionGuid)
  {
    ...
    return dbObject.CastToClass<DBDependency>();
  }
    ...
    Guid versionGuid)
  {
    ...
    Intermech.Diagnostics.Check.Result.NotNull<IDBObject>(dbObject);
    return (IDBObject) dbObject.CastToClass<DBDependency>();
  }

  IDBDependency IDBDependencyCollection.Create()
  {
    return ObjectExtensions.CastToInterface<IDBDependency>(this.Create(DBDependencyCollection.TypeID));
  }
}
