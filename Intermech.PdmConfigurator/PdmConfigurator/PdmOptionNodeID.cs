// Decompiled with JetBrains decompiler
// Type: Intermech.PdmConfigurator.PdmOptionNodeID
// Assembly: Intermech.PdmConfigurator, Version=7.0.2.1112, Culture=neutral, PublicKeyToken=null
// MVID: B5CB2E26-657B-4329-B46C-77AE46A32171
// Assembly location: D:\IPS\Client\Intermech.PdmConfigurator.dll

using Intermech.Navigator.Interfaces;
using System.Diagnostics;

#nullable disable
namespace Intermech.PdmConfigurator;

public sealed class PdmOptionNodeID : INodeID
{
  internal long _pdmOptionID;
  private object _cookie;

  public PdmOptionNodeID()
  {
  }

  public PdmOptionNodeID(long optionID) => this._pdmOptionID = optionID;

  public int CategoryID
  {
    [DebuggerStepThrough] get => 1;
  }

  public int TypeID
  {
    [DebuggerStepThrough] get => 0;
  }

  public object Cookie
  {
    [DebuggerStepThrough] get => this._cookie;
    [DebuggerStepThrough] set => this._cookie = value;
  }

  public override bool Equals(object obj)
  {
    return obj is PdmOptionNodeID pdmOptionNodeId && this._pdmOptionID == pdmOptionNodeId._pdmOptionID;
  }

  public override int GetHashCode() => this._pdmOptionID.GetHashCode();
}
