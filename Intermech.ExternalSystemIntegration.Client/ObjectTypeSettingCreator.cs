...
// Type: Intermech.ExternalSystemIntegration.Client.ObjectTypeSettingCreator
...
internal class ObjectTypeSettingCreator : 
  IObjectCreatorRiderCustomService,
  IObjectCreatorCustomService
{
  private ObjectTypeSettingCreatorStepOne ocs1;
  ...
  public Dictionary<UserControl, int> AddPages(object CreatedObject, int propPageIndex)
  {
    ...
    {
      this.ocs1 = new ObjectTypeSettingCreatorStepOne(createdObject);
  ...
  private static List<int> GetAllObjectTypes()
  {
    return new List<int>()
    {
      MetaDataHelper.GetObjectTypeID(Const.TypeSettingItemObjTypeGuid)
    };
  }

  public static void Attach(IObjectCreatorService service)
  {
    ObjectTypeSettingCreator._attachedObjectTypes = ObjectTypeSettingCreator.GetAllObjectTypes();
    foreach (int attachedObjectType in ObjectTypeSettingCreator._attachedObjectTypes)
      service.RegisterCreatorCustomService(attachedObjectType, typeof (ObjectTypeSettingCreator));
  }

  public static void Detach(IObjectCreatorService service)
  {
    if (ObjectTypeSettingCreator._attachedObjectTypes == null || ObjectTypeSettingCreator._attachedObjectTypes.Count <= 0)
      return;
    foreach (int attachedObjectType in ObjectTypeSettingCreator._attachedObjectTypes)
      service.UnregisterCreatorCustomService(attachedObjectType, typeof (ObjectTypeSettingCreator));
  }
}
