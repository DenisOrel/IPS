...
// Type: Intermech.Serialization.ClassicFormatters.Binary.BinaryFormatterWriter
...
internal sealed class BinaryFormatterWriter
{
  ...
  {
    this.WriteInt64(new DateTimeRaw()
    {
      DateTimeValue = value
    }.LongValue);
  }
    ...
    WriteObjectInfo[] memberObjectInfos)
  {
    ...
    string str = objectId < 0 ? typeNameInfo.NIname : nameInfo.NIname;
    ...
    if (this._objectMapTable.TryGetValue(str, out objectMapInfo) && objectMapInfo.IsCompatible(numMembers, memberNames, memberTypes))
    {
    ...
    else if (!typeNameInfo._transmitTypeOnObject)
    {
      ...
      this._binaryObjectWithMap.Set(objectId, str, numMembers, memberNames, assemId);
      this._binaryObjectWithMap.Write(this);
      if (objectMapInfo != null)
        return;
      this._objectMapTable.Add(str, new BinaryFormatterWriter.ObjectMapInfo(objectId, numMembers, memberNames, memberTypes));
    }
    else
    {
      ...
      this._binaryObjectWithMapTyped.Set(objectId, str, numMembers, memberNames, binaryTypeEnumA, typeInformationA, memberAssemIds, assemId);
  ...
  private void WriteArrayAsBytes(Array array, int typeLength)
  {
    ...
    int num2;
    for (; num1 < array.Length; num1 += num2)
    {
      num2 = Math.Min(4096 /*0x1000*/ / typeLength, array.Length - num1);
      int num3 = num2 * typeLength;
      Buffer.BlockCopy(array, num1 * typeLength, (Array) this._byteBuffer, 0, num3);
      if (!BitConverter.IsLittleEndian)
      {
        for (int index1 = 0; index1 < num3; index1 += typeLength)
        {
          for (int index2 = 0; index2 < typeLength / 2; ++index2)
          {
            byte num4 = this._byteBuffer[index1 + index2];
            this._byteBuffer[index1 + index2] = this._byteBuffer[index1 + typeLength - 1 - index2];
            this._byteBuffer[index1 + typeLength - 1 - index2] = num4;
          }
        }
      }
      this.WriteBytes(this._byteBuffer, 0, num3);
    }
  }
